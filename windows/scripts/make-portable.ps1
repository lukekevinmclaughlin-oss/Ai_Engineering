...
$application = Join-Path $stage "AI Engineering"
$artifact = Join-Path $release "AI Engineering-1.0.0-Windows-x64-Portable.zip"
$blockedWrapper = Join-Path $release "AI Engineering-1.0.0-Windows-x64-Portable.exe"
...
if (-not (Test-Path -LiteralPath (Join-Path $source "AI Engineering.exe"))) {
...
Push-Location $stage
try {
    & tar.exe -a -cf $artifact "AI Engineering"
