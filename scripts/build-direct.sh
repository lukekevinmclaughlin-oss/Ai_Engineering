...
build_root="${AI_ENGINEERING_DIRECT_BUILD_ROOT:-/private/tmp/macossoftware-ai-engineering-direct}"
derived_data="$build_root/DerivedData"
source_dir="$build_root/src"
stage_dir="$build_root/stage"
output_dir="${AI_ENGINEERING_DIRECT_OUTPUT_DIR:-/Users/$USER/Desktop/MacOSSoftware Direct Releases/AI Engineering}"
...
[[ "$build_root" == /private/tmp/macossoftware-ai-engineering-direct* || -n "${AI_ENGINEERING_DIRECT_BUILD_ROOT:-}" ]] || {
...
app_path="$derived_data/Build/Products/Direct/Ai_Engineering.app"
[[ -d "$app_path" ]] || { echo "Ai_Engineering.app was not produced" >&2; exit 1; }

bundle_id="$(defaults read "$app_path/Contents/Info" CFBundleIdentifier)"
[[ "$bundle_id" == "com.lukemclaughlin.aiengineering.direct" ]] || {
  echo "Unexpected direct bundle identifier: $bundle_id" >&2
  exit 1
}

executable="$app_path/Contents/MacOS/Ai_Engineering"
if otool -L "$executable" | grep -q 'StoreKit.framework'; then
  echo "Refusing to package a website build linked to StoreKit" >&2
  exit 1
fi
if rg -a -q 'aiengineering.pro.monthly|Restore purchases|Payment is charged to your Apple Account' "$executable"; then
  echo "Refusing to package a website build containing recurring-purchase code or UI" >&2
  exit 1
fi
...
dmg_path="$output_dir/Ai_Engineering-${version}-direct.dmg"
rm -f "$dmg_path"
hdiutil create -volname "Ai_Engineering Direct" -srcfolder "$stage_dir" -ov -format UDZO "$dmg_path"
